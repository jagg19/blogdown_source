...
coverImage: https://res.cloudinary.com/dyackvnwm/image/upload/v1544568681/A1MAIN-crazybull-042718-.jpg
coverSize: partial
...
- Yield Curve inversions typically leading indicator for Recession
- Historic analysis on inversions for specific yields since 1990 
- Yield Curve over 5 different time frames since start of 2018
- Takeaway from recent inversions


#Yield Curve Inversion
I've always followed bank stocks ever since I first began investing almost 6 years ago. Once I learned the 10-year and 2-year treasury spread was a profit driver for the big banks, I began to track the daily fluctuations. These two interest rates
are part of the larger yield curve that generally trends higher along the x-axis of a graph, while a "inverted" curve trends lower along the x-axis. The yield curve is widely spoken about so I will not get into too much detail but due to the inversion that first occured earlier this month (december 3rd 2018), it is once again very popular in mainstream media. I've had this code that I wrote, posted and updated to github a few months back, that I use daily to view and analyze the daily movements. I've decided to make a quick post to visualize the yield curve over five different time frames since the year has started and to also look at how many times and when specific yields inverted.
...
#setwd("/Users/jagvill/Desktop/R/Personal")
my_yc <- read.csv("yc_19902017.csv", stringsAsFactors = F)
#setwd("/Users/jagvill/Desktop/Github/blogdown_source")

#yc18 <- getYieldCurve("2018")[[2]]
#write.csv(yc18, "yc_2018.csv")
yc18 <- read.csv("yc_2018.csv", stringsAsFactors = F) 
...
Once its loaded, I can begin data manipulation to gather the appropiate yields to further analyze. Let us first add a column of the ten/two year spread since I would like to see this daily. Once we do this, our dataset dimensions are `r dim(my_yc)`. That is over 94,000 observations that we have loaded in. Then, lets look at the YTD graph and historical of the 10/2-year spread to see how if there is any interesting trends.
```{r echo=FALSE}
...
On 12-3-2018 we saw the 3-year yield and 5-year yield invert for the first time since 2007-06-05. Then the next set of yields to invert were the 2-year yield and 5-year yield on 12-4-2018 which has not inverted since 2007-06-06.
```{r First inversions since}
...
num_inversion_plot <- function(data){
  ...
  t <- data %>% select(ref_date,noquote(indx[i]),noquote(indx[b])) %>% subset(.[2] > .[3]) %>% {count(.,date = format(.$ref_date, "%Y-%m"))} %>% ggplot(aes(date,n)) + geom_col(fill = "lightblue") + theme(axis.text.x = element_text(angle = 60, hjust = 1)) + labs(title = paste("# of inversions for",indx[i],"&",indx[b], sep = " "), y = "Occurrences")
  print(t)
    }
  }
  
  ten_two <- data %>% select(ref_date, BC_2YEAR,BC_10YEAR) %>% filter(BC_2YEAR > BC_10YEAR) %>% {count(.,date = format(.$ref_date, "%Y-%m"))} %>% ggplot(aes(date, n)) + geom_col(fill = "lightblue") + labs(title = "# of inversions for BC_2YEAR & BC_10YEAR", y = "Occurrences")+ theme(axis.text.x = element_text(angle = 60, hjust = 1))
  print(ten_two)
  
  ten_thr_two <- data %>% select(ref_date,BC_2YEAR, BC_10YEAR,BC_30YEAR) %>% filter(BC_2YEAR > BC_10YEAR & BC_10YEAR > BC_30YEAR) %>% {count(.,date = format(.$ref_date, "%Y-%m"))} %>% ggplot(aes(date, n)) + geom_col(fill = "lightblue") + labs(title = "# of complete yield inversions - 2yr > 10yr > 30yr", y = "Occurrences") + theme(axis.text.x = element_text(angle = 60, hjust = 1))
  print(ten_thr_two)
}
...
Now, let us view the way the yield curve has changed since the start of 2018. Maybe this can paint a better pictue by showing us the momentum in which the yield curve has been inverting. We will take 5 different timeframes to plot our yield curve by using the seq and floor functions and setting the number of periods to 5. First, we need to convert our dataframe from "wide" format to "long" format, meaning we make all our columns their unique own rows with corresponding value's. This will take our dimensions of the dataframe from `r dim(my_yc)` to `r dim(my_yc_gather)` with dates being the index. This will allow us to index by our unique dates for each yield on the yield curve. Once we index by our dates, we can then plot away with our unique sequence yield curve to view YTD changes!
...
Most of the inversions happended in the years of 1990, 2000, and 2006-2007. The post savings and loan crisis in 1990, the dotcom bubble in 2000, and the financial crisis of 2007. All three timeframes were during recessions but did all result in a bear market (defined as a decline of 20%)? In theory, the answer is no. The only +20% market declines during these three year periods were in 2000 and 2007. Although, if we analyze a little closer we can see the answer *should* be YES! **If** we were to classify a "Bear" market as a decline of +19.9%, *only* 10 basis under the current 20% classification, then yes; all three year periods resulted in a Bear market (July-October/1990 incurred 19.92% loss). 
...
I do not believe we will enter a bear market due to the recent inversions in the yield curve. If you happen to be in the camp that does believe a bear market is coming, then you most likely believe the U.S and China trade spat will only worsen from here. This could mean that no deal is reached for a long period of time, additional tariffs, or a unfavorable deal is reached. I believe it is extremely hard to predict what two countries will settle on, when a deal is reached, or how it will affect the markets today. This is why I prefer buying into value and playing the long game rather than trying to predict how yield curve inversions and investor sentiment can affect markets short-term. 
