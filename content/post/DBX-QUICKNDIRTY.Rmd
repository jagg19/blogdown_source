...
Dropbox's shares have fallen -29.8% from its July high of $26.20, which was under the recent February high of $26.49 as of 09/04/2019. It's also -57.75% under the ATH of $43.50, so I saw this as a perfect opportunity to leverage R and run a "Quick N' Clean" valuation to answer a couple questions like "What sets Dropbox Apart" and more. 
...
# Function to calculate shares outstanding
#++++++++++++++++++++++++++++++++++++
...
price.book <- function(symbol, bvps) {
  ...
  a <- last(colnames(bvps))
  
  symbol_prices <- AlpacaforR::get_bars(symbol,from = as.Date(a)-5,to = as.Date(a)+5) %>% 
                                              map_df(data.frame) %>% filter(time < a) %>% 
                                              tail(1)
...
This is no surprise though. It should be expected that as a software company, it will have a high PB ratio (Mr. Buffet wouldn't touch this type of name). According to [NYU Sterns P/B ratio by Sector](http://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/pbvdata.html), the average between Software (Internet) and Software (System & Application) is 11.03 which Dropbox has a higher ratio of 14.6857.
...
price <- AlpacaforR::get_bars(ticker,from = "2019-09-04",to = "2019-09-04") %>% map_df(data.frame)
...
$26.46 at the Estimated Full-Year Revenue Guidance for 2019 and $24.46 at the current LTM Revenue which is both significantly above the most recent close price of $18.97. 
